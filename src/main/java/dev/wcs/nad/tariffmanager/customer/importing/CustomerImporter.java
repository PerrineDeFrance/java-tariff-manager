        ...
        try {
            ...
            switch (type.toUpperCase()) {
                case "E": {
                    ...
                    // SpecialCustomer specialCustomer = new SpecialCustomer(id, name, email,
                    // birthDate, lastBuyDate);
                    // return specialCustomer;

                    boolean lastPurchaseIn15Days = Period.between(lastBuyDate, LocalDate.now()).getDays() < 15;
                    if (lastPurchaseIn15Days) {
                        SpecialCustomer specialCustomer = new SpecialCustomer(specialCustomerDiscountPercentage, id,
                                name, email, birthDate, lastBuyDate);
                        return specialCustomer;
                    }
